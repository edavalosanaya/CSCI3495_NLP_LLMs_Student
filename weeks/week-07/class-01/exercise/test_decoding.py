...
def test_given_near_zero_temperature_is_greedy():
    dist = dec.apply_temperature(LOGITS, 1e-9)
    assert math.isclose(dist["a"], 1.0, rel_tol=1e-6)
    assert math.isclose(sum(dist.values()), 1.0, rel_tol=1e-6)


def test_given_greedy_picks_argmax():
    dist = dec.apply_temperature(LOGITS, 1.0)
    assert dec.greedy(dist) == "a"


def test_step1_top_k_keeps_k_and_normalizes():
    dist = dec.apply_temperature(LOGITS, 1.0)
...
def test_step2_top_p_keeps_nucleus():
...
def test_step2_top_p_always_keeps_at_least_one():
    dist = {"a": 0.5, "b": 0.3, "c": 0.2}
    filt = dec.top_p_filter(dist, 0.0)
    assert len(filt) >= 1


def test_given_sample_is_deterministic_with_seed():
    dist = dec.apply_temperature(LOGITS, 1.0)
...
def test_given_sample_respects_certainty():
