# W13C2 Walkthrough: Mini RAG, step by step
...
**This is W5C1's search engine with sklearn doing the TF-IDF.** Point that out:
students built this by hand in Week 5, including the cosine and the tie-break.
...
is the same instinct as the unanswerable item in W11C2's dataset: you have to make
...
met in W5C1 and W5C2. Real systems apply a score threshold.
...
**Connecting to the CTF next door.** W13C1's lesson was that model output must be
validated before anything downstream trusts it. `verify_citations` is that same
control applied to a claim of evidence. And the stretch goal in W13C1 (indirect
